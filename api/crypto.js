// Vercel serverless function — whale positions grouped by crypto market
export default async function handler(req, res) {
  ...
  // Reject hex addresses / conditionId strings as display names
  function sanitizeName(raw) {
    if (!raw) return null;
    if (/^0x[0-9a-fA-F]{8}/i.test(raw)) return null; // looks like address/conditionId
    if (raw.length > 42) return null; // suspiciously long
    return raw;
  }
  ...
  // Fetch open crypto positions for all whales in parallel
