// Vercel serverless function — daily whale digest email + health check
// Cron 1: 0 14 * * * → daily email digest (2 PM UTC)
// Cron 2: 0 8  * * * → health check (?mode=health)
import { Resend } from 'resend';

// ── Health check helpers ──────────────────────────────────────────────────────
const HEALTH_BASE = 'https://whaletrack.app';
const HEALTH_PAGES = [
...
async function checkPage({ url, label, keyword }) {
  try {
    const r = await fetch(HEALTH_BASE + url, { signal: AbortSignal.timeout(10000) });
  ...
  } catch (e) { return { label, ok: false, reason: e.message || 'Timeout / network error' }; }
}

async function checkApi({ url, label }) {
  try {
    const r = await fetch(HEALTH_BASE + url, { signal: AbortSignal.timeout(15000) });
    if (!r.ok) return { label, ok: false, reason: `HTTP ${r.status}` };
    const data = await r.json();
    if (!Array.isArray(data) && typeof data !== 'object') return { label, ok: false, reason: 'Not valid JSON' };
    return { label, ok: true, count: Array.isArray(data) ? data.length : null };
  } catch (e) { return { label, ok: false, reason: e.message || 'Timeout / network error' }; }
}

async function sendHealthAlert(failures, results) {
  const RESEND_API_KEY = process.env.RESEND_API_KEY;
  if (!RESEND_API_KEY) return;
  const failureRows = failures.map(f =>
    `<tr style="background:#1c0a0a"><td style="padding:8px 12px;color:#f85149;font-weight:700">❌ ${f.label}</td><td style="padding:8px 12px;color:#e6edf3">${f.reason}</td></tr>`
  ).join('');
  const allRows = results.map(r =>
    `<tr><td style="padding:6px 12px;color:${r.ok ? '#3fb950' : '#f85149'}">${r.ok ? '✅' : '❌'} ${r.label}</td><td style="padding:6px 12px;color:#8b949e">${r.ok ? (r.count !== null ? `${r.count} items` : 'OK') : r.reason}</td></tr>`
  ).join('');
  const html = `<div style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;background:#0d1117;padding:24px;border-radius:12px;max-width:600px">
    ...
    <table style="width:100%;border-collapse:collapse;margin-bottom:20px;font-size:13px">${failureRows}</table>
    <h3 style="color:#e6edf3;font-size:14px;margin:0 0 8px">📋 Full Report</h3>
    <table style="width:100%;border-collapse:collapse;font-size:13px">${allRows}</table>
...
export default async function handler(req, res) {
  ...
  // ── Health check mode (?mode=health) ─────────────────────────────────────
  if (req.query.mode === 'health') {
    const checks = [
      ...HEALTH_PAGES.map(p => checkPage(p)),
      ...HEALTH_APIS.map(a => checkApi(a)),
    ];
    const results = await Promise.all(checks);
    const failures = results.filter(r => !r.ok);
    if (failures.length > 0) await sendHealthAlert(failures, results);
